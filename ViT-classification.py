...
from util.datasets import build_dataset, build_dataset_padding
...
def get_args_parser():
    parser = argparse.ArgumentParser('ViT for image classification', add_help=False)
    ...
    # * Finetuning params
    parser.add_argument('--pretrained', default='/prj/ViT-pytorch/checkpoints/imagenet21k/ViT-L_16.npz',
                        help='transfer learning from checkpoint')
    ...
    # Dataset parameters
    parser.add_argument('--dataset', default='diatom_v', type=str,
                        help='dataset name')
    parser.add_argument('--focal_loss', default=False,
                        help='whether use focal loss ')
    parser.add_argument('--p_focal_loss', default=False,
                        help='whether use focal loss ')
    parser.add_argument('--gamma', type=float, default=2.0,
                        help='gamma value for focal loss ')
    parser.add_argument('--data_path', default='/prj/SSL/freshwater_diatom/finetune/cls38_t_v/', type=str,
                        help='dataset path')
...
def main(args):
    ...
    np.random.seed(seed)
    torch.cuda.manual_seed(seed)  # current gpu Sets the seed for generating random numbers for the current GPU.
    ...
    if args.pretrained and not args.eval:
    ...
    if args.eval:
        test_stats, cf_matrix = evaluate(dataset_val, data_loader_val, model, device, args)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            ...
            prob_focal_loss=args.p_focal_loss,
            gamma=args.gamma
        )
